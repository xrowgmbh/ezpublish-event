    <div id="ezpeventperiod_{$index}" class="ezpeventexcludeperiod">
{if $index|gt( 0 )}<hr class="ezpeventhr" />{/if}
        ...
        <label for="startdate" class="labelstartdate">
            ...
            <input class="ezpublisheventdate" id="startdate_{$index}" value="{if is_set( $item )}{if and( is_set( $item.startdate ), $item.startdate|ne('') )}{$item.startdate}{/if}{/if}" type="text" size="40" maxlength="40" name="{$attribute_base}_ezpeventdate_data_{$attribute.id}[include][{$index}][startdate]" placeholder="{'Choose date'|i18n( 'extension/ezpublish-event' )}" />
        </label>
        ...
        </label><span>{"o'clock"|i18n( "extension/ezpublish-event" )}</span>
        <div class="weekdays" id="ezpeventperiodweekdays_{$index}"{if or( and( is_set( $item.weekdays ), $item.weekdays|count|gt( 0 ), $item.weekdays|count|lt( 7 ) ), is_set( $postDataDays[$index] ) )} style="display: block"{/if}>
            ...
            <label for="Wednesday" class="labelweekday">
                <input type="checkbox" {if or( is_set( $item )|not(), and( is_set( $item ), or( and( is_set( $item.weekdays ), $item.weekdays|contains( 'Wed' ) ), is_set( $item.weekdays )|not() ) ) )}checked {/if}size="3" maxlength="2" name="{$attribute_base}_ezpeventdate_data_{$attribute.id}[include][{$index}][weekdays][2]" value="Wen">{'Wednesday'|i18n( 'extension/ezpublish-event' )}
            </label>
            ...
            <label for="Saturday" class="labelweekday">
                <input type="checkbox" {if or( is_set( $item )|not(), and( is_set( $item ), or( and( is_set( $item.weekdays ), $item.weekdays|contains( 'Sat' ) ), is_set( $item.weekdays )|not() ) ) )}checked {/if}size="3" maxlength="2" name="{$attribute_base}_ezpeventdate_data_{$attribute.id}[include][{$index}][weekdays][5]" value="Sat">{'Saturday'|i18n( 'design/admin/content/translationview' )}
            </label>
            <label for="Sunday" class="labelweekday lastelement">
                <input type="checkbox" {if or( is_set( $item )|not(), and( is_set( $item ), or( and( is_set( $item.weekdays ), $item.weekdays|contains( 'Son' ) ), is_set( $item.weekdays )|not() ) ) )}checked {/if}size="3" maxlength="2" name="{$attribute_base}_ezpeventdate_data_{$attribute.id}[include][{$index}][weekdays][6]" value="Son">{'Sunday'|i18n( 'design/admin/content/translationview' )}
            </label>
