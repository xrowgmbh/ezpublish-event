...
{if $index|gt( 0 )}<hr class="ezpeventhr" />{/if}
        ...
        <div class="weekdays" id="ezpeventperiodweekdays_{$index}"{if and( is_set( $days ), $days|ge( 3 ) )} style="display: block"{/if}>
            ...
            <label for="Monday" class="labelweekday">
                <input type="checkbox" {if or( is_set( $item )|not(), and( is_set( $item ), or( and( is_set( $item.weekdays ), or( and( is_set( $item.weekdays.0 ), $item.weekdays.0|eq('') ), $item.weekdays|contains( $weekdayShortNames[0] ) ) ), is_set( $item.weekdays )|not() ) ) )}checked {/if}size="3" maxlength="2" name="{$attribute_base}_ezpeventdate_data_{$attribute.id}[include][{$index}][weekdays][0]" value="{$weekdayShortNames[0]}">{'Monday'|i18n( 'design/admin/content/translationview' )}
            </label>
            <label for="Tuesday" class="labelweekday">
                <input type="checkbox" {if or( is_set( $item )|not(), and( is_set( $item ), or( and( is_set( $item.weekdays ), or( and( is_set( $item.weekdays.1 ), $item.weekdays.1|eq('') ), $item.weekdays|contains( $weekdayShortNames[1] ) ) ), is_set( $item.weekdays )|not() ) ) )}checked {/if}size="3" maxlength="2" name="{$attribute_base}_ezpeventdate_data_{$attribute.id}[include][{$index}][weekdays][1]" value="{$weekdayShortNames[1]}">{'Tuesday'|i18n( 'extension/ezpublish-event' )}
            </label>
            <label for="Wednesday" class="labelweekday">
                <input type="checkbox" {if or( is_set( $item )|not(), and( is_set( $item ), or( and( is_set( $item.weekdays ), or( and( is_set( $item.weekdays.2 ), $item.weekdays.2|eq('') ), $item.weekdays|contains( $weekdayShortNames[2] ) ) ), is_set( $item.weekdays )|not() ) ) )}checked {/if}size="3" maxlength="2" name="{$attribute_base}_ezpeventdate_data_{$attribute.id}[include][{$index}][weekdays][2]" value="{$weekdayShortNames[2]}">{'Wednesday'|i18n( 'extension/ezpublish-event' )}
            </label>
            <label for="Thursday" class="labelweekday">
                <input type="checkbox" {if or( is_set( $item )|not(), and( is_set( $item ), or( and( is_set( $item.weekdays ), or( and( is_set( $item.weekdays.3 ), $item.weekdays.3|eq('') ), $item.weekdays|contains( $weekdayShortNames[3] ) ) ), is_set( $item.weekdays )|not() ) ) )}checked {/if}size="3" maxlength="2" name="{$attribute_base}_ezpeventdate_data_{$attribute.id}[include][{$index}][weekdays][3]" value="{$weekdayShortNames[3]}">{'Thursday'|i18n( 'extension/ezpublish-event' )}
            </label>
            <label for="Friday" class="labelweekday">
                <input type="checkbox" {if or( is_set( $item )|not(), and( is_set( $item ), or( and( is_set( $item.weekdays ), or( and( is_set( $item.weekdays.4 ), $item.weekdays.4|eq('') ), $item.weekdays|contains( $weekdayShortNames[4] ) ) ), is_set( $item.weekdays )|not() ) ) )}checked {/if}size="3" maxlength="2" name="{$attribute_base}_ezpeventdate_data_{$attribute.id}[include][{$index}][weekdays][4]" value="{$weekdayShortNames[4]}">{'Friday'|i18n( 'extension/ezpublish-event' )}
            </label>
            <label for="Saturday" class="labelweekday">
                <input type="checkbox" {if or( is_set( $item )|not(), and( is_set( $item ), or( and( is_set( $item.weekdays ), or( and( is_set( $item.weekdays.5 ), $item.weekdays.5|eq('') ), $item.weekdays|contains( $weekdayShortNames[5] ) ) ), is_set( $item.weekdays )|not() ) ) )}checked {/if}size="3" maxlength="2" name="{$attribute_base}_ezpeventdate_data_{$attribute.id}[include][{$index}][weekdays][5]" value="{$weekdayShortNames[5]}">{'Saturday'|i18n( 'extension/ezpublish-event' )}
            </label>
            <label for="Sunday" class="labelweekday lastelement">
                <input type="checkbox" {if or( is_set( $item )|not(), and( is_set( $item ), or( and( is_set( $item.weekdays ), or( and( is_set( $item.weekdays.6 ), $item.weekdays.6|eq('') ), $item.weekdays|contains( $weekdayShortNames[6] ) ) ), is_set( $item.weekdays )|not() ) ) )}checked {/if}size="3" maxlength="2" name="{$attribute_base}_ezpeventdate_data_{$attribute.id}[include][{$index}][weekdays][6]" value="{$weekdayShortNames[6]}">{'Sunday'|i18n( 'design/admin/content/translationview' )}
            </label>
